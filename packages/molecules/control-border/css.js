...
const AtrcControlBorderCss = (
	...
	if (!isEmpty(value)) {
		...
		if (value.cl) {
			...
			output.xs += `${properties.bdrCl}:${AtrcControlBorderColorShorthand(
				value.tCl,
				value.rCl,
				value.bCl,
				value.lCl
		...
		if (value.sty) {
			...
			output.xs += `${properties.bdrSty}:${AtrcControlBorderStyleShorthand(
				value.tSty,
				value.rSty,
				value.bSty,
				value.lSty
		...
		if (value.w) {
			...
			output.xs += `${properties.bdrW}:${AtrcControlBorderWidthShorthand(
				value.tW,
				value.rW,
				value.bW,
				value.lW
