...
const AtrcGoogleMap = ({
  ...
  const [msg, setMessage] = useState(apiKey ? __('Script is not loaded', 'atrc-prefix-atrc') : '');
  let gMap;
  ...
  function addInfoWindow(marker, markerId, title, description) {
    const contentString = `<div class="at-map-overview"><h6 class="at-map-overview-title">${title}</h6><div class="at-map-overview-content">${description ? `<p>${description}</p>` : ''}<a class="at-map-overview-delete" onclick="removeMarker( '${markerId}' )">${__('Delete Marker', 'atrc-prefix-atrc')}</a></div></div>`;
  ...
  return apiKey || msg ? /*#__PURE__*/React.createElement("div", _extends({
    ...
    src: `https://maps.google.com/maps?q=${encodeURIComponent(loc)}&t=&z=${zoom}&ie=UTF8&output=embed`,
    frameBorder: "0",
    className: classNames(className, 'at-map'),
    id: mapID
  }, defaultProps));
