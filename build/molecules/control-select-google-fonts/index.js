...
const AtrcControlSelectGoogleFonts = props => {
  ...
  const fontOptions = useMemo(() => {
    return fonts && fonts.length ? fonts.map(item => ({
      label: item.family,
      value: item.family
    })) : [];
  }, [fonts]);
  useEffect(() => {
    setFilteredOptions(fonts && fonts.length ? fonts.map(item => ({
      label: item.family,
      value: item.family
    })) : []);
  }, [fontOptions]);
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(AtrcPanelRow, {
    ...
    onFilterValueChange: inputValue => setFilteredOptions(fontOptions.filter(option => option.label.toLowerCase().startsWith(inputValue.toLowerCase())))
...
export { AtrcControlSelectGoogleFontsWeight, AtrcControlSelectGoogleFontsPreview };
