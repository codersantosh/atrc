...
/*Local*/
const AtrcControlTransformTabCss = (value, properties = {
  tf: '--at-tf',
  ppv: '--at-ppv',
  ppvOrg: '--at-ppv-org',
  tfOrg: '--at-tf-org',
  ...
  if (!isEmpty(value)) {
    AtrcAvailableTabs.forEach(tab => {
      if (AtrcAvailableTabs.includes(tab)) {
        if (AtrcHasTabValues(tab, value, AtrcControlTransformAllowedKeys)) {
          const transform = AtrcGetTabValues(value, tab, AtrcControlTransformAllowedKeys),
    ...
    if (value.orgX || value.orgY || value.orgZ) {
      if (value.on3D) {
        output.xs += `${properties.tfOrg} : ${value.orgX || 0} ${value.orgY || 0} ${value.orgZ || 0};`;
        output.xs += ``;
      } else {
        output.xs += `${properties.tfOrg} :${value.orgX || 0} ${value.orgY || 0}`;
      }
    }
  }
  return output;
