...
const AtrcControlCustomAttributes = props => {
  ...
  const [inputValue, setInputValue] = useState(Object.entries(value).map(([name, val]) => `${name}='${val}'`).join(' '));
  ...
  const handleBlur = () => {
    ...
    const attributeRegex = /\b([\w-]+?)=['"](.+?)['"]|\b([\w-]+?)=['](.+?)[']|\b([\w-]+)/g;
    ...
    if (matches.length === 0) {
      ...
      setError(`Attribute(s) "${duplicateAttributes.join(', ')}" are used more than once`);
    } else if (disallowedAttributes.length > 0) {
      setError(`Attribute(s) "${disallowedAttributes.join(', ')}" are not allowed`);
    } else {
      setError('');
    }
