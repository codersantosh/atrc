...
const AtrcControlPositionDeviceTab = props => {
  ...
  const AllTabs = useMemo(() => {
    ...
    if (allowedTabs.includes('normal'.toLowerCase())) {
      tabs.push({
        ...
        onDeselect: () => onChange(AtrcResetTab('normal', value, AtrcControlPositionAllowedKeys))
      });
    }
    if (allowedTabs.includes('hover'.toLowerCase())) {
      tabs.push({
        ...
        onDeselect: () => onChange(AtrcResetTab('Hover', value, AtrcControlPositionAllowedKeys))
      });
    }
    if (allowedTabs.includes('parenthover'.toLowerCase())) {
      tabs.push({
        ...
        hasValue: AtrcHasTabValues('ParentHover', value, AtrcControlPositionAllowedKeys),
        onDeselect: () => onChange(AtrcResetTab('ParentHover', value, AtrcControlPositionAllowedKeys))
      });
    }
    return tabs;
  }, []);
  return /*#__PURE__*/React.createElement(AtrcPanelTools, {
    ...
    label: label,
    resetAll: () => onChange(AtrcResetTabs(value, AtrcControlPositionAllowedKeys)),
    allowTabs: true,
    tools: AllTabs
