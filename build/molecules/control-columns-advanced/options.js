...
const CustomColumnsExistingValue = ({
  value,
  colNum
}) => {
  const splitedValue = value.split('-');
  const splitedValueTotal = splitedValue.reduce((total, num) => total + parseInt(num), 0);
  const MAX_VALUE = 12; // Maximum value for comparison

  if (splitedValueTotal === MAX_VALUE) {
    const customWidth = splitedValue.map(num => {
...
const NewValWithCustom = ({
  newVal,
  NewExistingValue
}) => {
  if (Array.isArray(newVal)) {
    const width = newVal.map((num, i) => {
      if (i > 0) {
...
const AllColumnsOptions = [...OneColumnOptions, ...TwoColumnsOptions, ...ThreeColumnsOptions, ...FourColumnsOptions, ...FiveColumnsOptions, ...SixColumnsOptions];

/*Export Everything*/
export { OneColumnOptions, TwoColumnsOptions, ThreeColumnsOptions, FourColumnsOptions, FiveColumnsOptions, SixColumnsOptions, AllColumnsOptions, CustomColumnOption, CustomColumnsExistingValue, NewValWithCustom };
