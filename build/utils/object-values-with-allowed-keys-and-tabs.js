...
export const AtrcGetTabValues = (value, tab, allowedTabKeys = []) => {
  ...
  const tabKeys = tab === 'normal' ? allowedTabKeys : allowedTabKeys.map(prop => prop + tab);
  return mapKeys(pick(value, tabKeys), (val, key) => tab === 'normal' ? key : key.replace(tab, ''));
};
export const AtrcUpdateTabValues = (newVal, tab, value, allowedTabKeys = []) => {
...
export const AtrcHasTabValues = (tab, value, allowedTabKeys = []) => {
  ...
  return allowedTabKeys.some(key => AtrcIsKeyWithValueNotEmpty(value, `${key}${tab}`));
};
...
export const AtrcResetTabs = (value, allowedTabKeys) => {
  ...
  if (!isArray(allowedTabKeys)) {
    throw new Error(`Invalid allowedTabKeys: ${allowedTabKeys}. Expected an array.`);
  }
  let newValue = {
    ...value
  };
